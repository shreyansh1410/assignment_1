...
import { AboutMe, Experiences, Recommended } from "./TabContent";
import "./TabNavigation.css";

function TabNavigation({ activeTab, setActiveTab }) {
  return (
    <div
      className="w-full rounded-lg p-4 shadow-xl flex items-start justify-between mt-1"
    ...
    >
      ...
      <div className="flex flex-col flex-1 ml-4 w-full">
        <div className="w-full flex items-center justify-between mb-4">
          <div className="w-full flex items-center space-x-4">
            <div
              className="w-full flex space-x-2 p-1 rounded-2xl justify-between"
            ...
            >
              {["aboutMe", "experiences", "recommended"].map((tab) => (
                ...
                className={`relative px-8 py-2 rounded-xl focus:outline-none transition-transform transform ${
                  activeTab === tab
                    ? "bg-[#28292f] text-white shadow-3xl transform scale-110"
                    : "bg-[#171717] text-gray-300"
                }`}
                style={
                  activeTab === tab
                    ? {
                        boxShadow: "0 0 30px rgba(0, 0, 0, 0.9), 0 0 60px rgba(0, 0, 0, 0.8), 0 0 90px rgba(0, 0, 0, 1)",
                        transform: "scale(1)"
                      }
                    : {}
                }
        ...
        <div
          className="scrollable-content h-48 overflow-y-auto text-gray-500 rounded-md p-4"
          style={{ backgroundColor: "#363c43" }}
        >
